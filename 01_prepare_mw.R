...
### 7. Add observed pollinator interactions and write the final "focal" metaweb
observed_interactions <- read.csv("data/01_raw/pollinator_interactions.csv") %>%
  select(-ugs) %>%
  distinct()

metaweb_focal <- rbind(metaweb_cleaned, observed_interactions) %>% unique()

write.csv(mw.focal, "data/02_processed/focal_metaweb.csv", row.names = F)